...
contract EmergencyUpgradeBoard is EIP712 {
    ...
    function executeEmergencyUpgrade(
        ...
        bytes32 guardiansDigest = _hashTypedDataV4(
            keccak256(abi.encode(EXECUTE_EMERGENCY_UPGRADE_GUARDIANS_TYPEHASH, id))
        );
        require(
            GUARDIANS.isValidERC1271SignatureNow(guardiansDigest, _guardiansSignatures),
            "Invalid guardians signatures"
        );

        bytes32 securityCouncilDigest = _hashTypedDataV4(
            keccak256(abi.encode(EXECUTE_EMERGENCY_UPGRADE_SECURITY_COUNCIL_TYPEHASH, id))
        );
        require(
        ...
        bytes32 zkFoundationDigest = _hashTypedDataV4(
            keccak256(abi.encode(EXECUTE_EMERGENCY_UPGRADE_ZK_FOUNDATION_TYPEHASH, id))
        );
        require(
